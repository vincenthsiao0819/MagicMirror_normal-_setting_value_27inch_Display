...
## 2026-06-19 補充更新 (Split Title)
為了讓特定事件（例如「民權國中-」開頭的事件）過長時能夠完整顯示，我們新增了 **`patch_split_title.js`**：
- 自動攔截包含 `民權國中-` 的字串。
- 將 `民權國中` 切割並獨立顯示為第一行（字體略小、顏色變淺 `color: #cccccc`）。
- 移除破折號 `-`，並把原本的內容（例如 `七八年級第3次定期評量...`）顯示在第二行。
- 同時放寬了 JS 端原有的 `maxTitleLength: 20` 字元擷取限制，擴增到 `40` 個字元，確保換行後仍能看見更多的文字細節。
